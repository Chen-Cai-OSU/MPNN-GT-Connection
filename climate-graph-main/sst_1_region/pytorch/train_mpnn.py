...
# Dataset
import xarray
from oisstv2_dataset import oisstv2_dataset, create_adj
...
assert virtual_node == 0 or virtual_node == 1
...
train_dataset = oisstv2_dataset(dataset = dataset, slice_index = train_slice, history_window = history_window, predict_window = predict_window, virtual_node = virtual_node)
valid_dataset = oisstv2_dataset(dataset = dataset, slice_index = valid_slice, history_window = history_window, predict_window = predict_window, virtual_node = virtual_node)
test_dataset = oisstv2_dataset(dataset = dataset, slice_index = test_slice, history_window = history_window, predict_window = predict_window, virtual_node = virtual_node)
...
assert num_nodes == num_longitudes * num_latitudes + virtual_node

indices, values = create_adj(num_longitudes, num_latitudes, virtual_node = virtual_node)
adj = (indices, values)
print('Done creating sparse adjacency matrix')

# Init model and optimizer
model = MPNN(num_layers = args.n_layers, node_dim = node_dim, edge_dim = None, hidden_dim = args.hidden_dim, z_dim = args.z_dim, num_outputs = num_outputs, regression = True, device = device).to(device = device)
...
for epoch in range(num_epoch):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        # Model
        predict, latent = model(adj, node_feat, edge_feat = None)
        optimizer.zero_grad()
        
        # If used virtual node
        if virtual_node == 1:
            predict = torch.flatten(predict[:, : predict.size(1) - 1, :])
...
with torch.no_grad():
    ...
    for batch_idx, data in enumerate(test_dataloader):
        ...
        # If use virtual node
        if virtual_node == 1:
            predict = predict[:, : predict.size(1) - 1, :]
