...
class oisstv2_dataset(Dataset):
    def __init__(self, dataset, slice_index, history_window = 7, predict_window = 7, virtual_node = 0):
    ...
    def __getitem__(self, idx):
        ...
        # Input
        inputs = torch.FloatTensor(self.array[idx : idx + self.history_window, :, :].to_numpy())
        inputs = inputs.transpose(0, 1).transpose(1, 2)
        inputs = torch.reshape(inputs, (inputs.size(0) * inputs.size(1), inputs.size(2)))
        
        # Targets
        targets = torch.FloatTensor(self.array[idx + self.history_window : finish, :, :].to_numpy())
