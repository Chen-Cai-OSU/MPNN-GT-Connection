...
def _parse_args():
    ...
    parser.add_argument('--window', '-window', type = int, default = 7, help = 'Window')
...
train_dataset = oisstv2_dataset(dataset = dataset, slice_index = train_slice, window = window)
valid_dataset = oisstv2_dataset(dataset = dataset, slice_index = valid_slice, window = window)
test_dataset = oisstv2_dataset(dataset = dataset, slice_index = test_slice, window = window)
...
time_range = 3
model = LES(input_channels = window - time_range, output_channels = 1, kernel_size = 3, dropout_rate = 0, time_range = time_range).to(device = device)
...
for epoch in range(num_epoch):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        images = torch.reshape(node_feat, (batch_size, num_longitudes, num_latitudes, window))
        images = images.transpose(2, 3).transpose(1, 2)

        # Pad with zeros to get 64x64
        images_ = torch.zeros(batch_size, window, num_longitudes + 4, num_latitudes + 4).to(device = device)
        images_[:, :, 2 : num_longitudes + 2, 2 : num_latitudes + 2] = images
        images = images_
        ...
        # Fix the prediction
        predict = torch.mean(predict, dim = 1).unsqueeze(dim = 1)

        # Remove the padded zeros
        predict = torch.reshape(predict[:, :, 2 : num_longitudes + 2, 2 : num_latitudes + 2], (batch_size, num_longitudes * num_latitudes, targets.size(2)))

        # Mean squared error loss
        loss = F.mse_loss(predict, targets, reduction = 'mean')

        sum_error += torch.sum(torch.abs(predict - targets)).detach().cpu().numpy()
