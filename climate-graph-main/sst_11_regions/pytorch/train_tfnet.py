...
# Pad with zeros for TF-Net
lon_pad_size = (64 - num_longitudes) // 2
lat_pad_size = (64 - num_latitudes) // 2

print('Pad size in longitudes for TF-Net:', lon_pad_size)
print('Pad size in latitudes for TF-Net:', lat_pad_size)

# Init model and optimizer
time_range = history_window // 2 # Time range is half the window
model = LES(input_channels = history_window - time_range, output_channels = predict_window, kernel_size = 3, dropout_rate = 0, time_range = time_range).to(device = device)
...
for epoch in range(num_epoch):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        images_ = torch.zeros(batch_size, history_window, num_longitudes + 2 * lon_pad_size, num_latitudes + 2 * lat_pad_size).to(device = device)
        images_[:, :, lon_pad_size : num_longitudes + lon_pad_size, lat_pad_size : num_latitudes + lat_pad_size] = images
        images = images_
        ...
        predict = torch.reshape(predict[:, :, lon_pad_size : num_longitudes + lon_pad_size, lat_pad_size : num_latitudes + lat_pad_size], (batch_size, predict_window, num_longitudes * num_latitudes))
...
np.save('TF-Net.predict', saved_predict)
np.save('TF-Net.target', saved_target)
