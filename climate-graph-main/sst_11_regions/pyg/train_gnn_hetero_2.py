...
# Dataset
import xarray
from oisstv2_dataset_pyg import read_all_raw_data, oisstv2_dataset_pyg, create_adj, create_adj_hetero

# Model
import sys
sys.path.insert(1, '../../models/pyg/')
from gnn_hetero_2 import GNN # Heterogeneous GNN
...
train_dataset = oisstv2_dataset_pyg(raw_datasets = raw_datasets, split = 'train', history_window = history_window, predict_window = predict_window, lon_filter = lon_filter, lat_filter = lat_filter, hetero = True)
valid_dataset = oisstv2_dataset_pyg(raw_datasets = raw_datasets, split = 'valid', history_window = history_window, predict_window = predict_window, lon_filter = lon_filter, lat_filter = lat_filter, hetero = True)
test_dataset = oisstv2_dataset_pyg(raw_datasets = raw_datasets, split = 'test', history_window = history_window, predict_window = predict_window, lon_filter = lon_filter, lat_filter = lat_filter, hetero = True)
...
assert valid_dataset.num_nodes == num_nodes
assert test_dataset.num_nodes == num_nodes
...
for epoch in range(num_epoch):
    ...
    for batch_idx, data in enumerate(train_dataloader):
        ...
        if args.pe == 'lap':
            batch_size = data.x.size(0) // num_nodes
            pe = torch.cat([eigenvectors for b in range(batch_size)], dim = 0)
...
with torch.no_grad():
    for batch_idx, data in enumerate(test_dataloader):
        ...
        # Reshape
        batch_size = data.x.size(0) // num_nodes
        target = torch.reshape(target, (batch_size, num_nodes, predict_window))
        predict = torch.reshape(predict, (batch_size, num_nodes, predict_window))

        # To save
        saved_target = torch.reshape(target[0], (num_longitudes, num_latitudes, predict_window)).detach().cpu().numpy()
...
if args.pe == 'lap':
    vis_name += '_PE'
np.save(vis_name + '.predict', saved_predict)
np.save(vis_name + '.target', saved_target)
