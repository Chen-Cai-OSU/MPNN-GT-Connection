...
class EigenTransform(BaseTransform):
    def __init__(self, pe_types=['LapPE']):
        self.pe_types = pe_types
        self.cfg = set_cfg()

    def _get_isundirected(self, dataset):
        is_undirected = True # all(d.is_undirected() for d in dataset[:10])
        return is_undirected

    def __call__(self, data: Data) -> Data:
        is_undirected = self._get_isundirected(data)
        data = compute_posenc_stats(data, self.pe_types, is_undirected, self.cfg)
        return data

    def __repr__(self) -> str:
        return f'{self.__class__.__name__}({self.pe_types})'
