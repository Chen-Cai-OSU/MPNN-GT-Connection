...
class NewCOCOSuperpixels(InMemoryDataset):
    r"""The COCOSuperpixels dataset which contains image superpixels and a semantic segmentation label
    for each node superpixel.
    ...
    def _sample(self, split, n):
        ...
        if self.suffix == 'small':
            epsilon = 0.1 if split == 'train' else 0.5
            indices = random.sample(indices, int(n * epsilon))
            print(f'first 5 indices for {split}: {indices[:5]}')
        elif self.suffix == 'tiny':
            epsilon = 0.001 if split == 'train' else 0.01
            indices = random.sample(indices, int(n * epsilon))
            print(f'first 5 indices for {split}: {indices[:5]}')
        else:
            raise NotImplementedError(self.suffix)
        return indices
...
if __name__ == '__main__':
    dataset = NewCOCOSuperpixels(suffix='small', root='./datasets')
