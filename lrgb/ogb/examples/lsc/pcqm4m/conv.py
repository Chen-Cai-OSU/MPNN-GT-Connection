...
class GNN_node_Virtualnode(torch.nn.Module):
    ...
    def __init__(self, num_layers, emb_dim, drop_ratio=0.5, JK="last", residual=False, gnn_type='gin'):
        ...
        self.pos_enc = LapPENodeEncoder(10, expand_x=False)
        self.atom_encoder = AtomEncoder(emb_dim - 10)
    ...
    def forward(self, batched_data):
        ...
        pre_mpnn_emb = self.atom_encoder(x)
        pos_emb = self.pos_enc(batched_data).x
        emb = torch.cat((pre_mpnn_emb, pos_emb), axis=1)
        h_list = [emb]
        for layer in range(self.num_layers):
