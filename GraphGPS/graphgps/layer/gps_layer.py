...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        if local_gnn_type == 'None':
            ...
            from graphgps.encoder.signnet_pos_encoder import MLP
            update_net = MLP(dim_h, dim_h, dim_h, 2,
                             use_bn=self.batch_norm, dropout=dropout, activation=vn_cfg.nonlinear)
            self.local_model = pygnn.GINConv(update_net)
        ...
        elif global_model_type == 'DSVN':
            self.self_attn = EquivariantDS(dim_h, vn_cfg.n_layers, reduction=vn_cfg.reduction, nonlinear=vn_cfg.nonlinear)
        else:
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            if self.local_gnn_type == 'CustomGatedGCN':
                ...
                if self.equivstable_pe:
                    ...
                    if self.local_gnn_type in ['GCNConv', 'GINConv']:
        ...
        if self.self_attn is not None:
            ...
            elif self.global_model_type == 'DSVN':
                h_attn = self.self_attn(h_dense, mask=mask)[mask]
            else:
